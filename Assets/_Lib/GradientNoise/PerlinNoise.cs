...
namespace NoiseLib
{
	...
	public class PerlinNoise : IHeightMappable<Vector2>
	{
		...
		public float noise(Vector2 p) {
			...
			float x00 = M.Dot (_00, dx0, dy0);     // u = p dot h[p_00]
			float x10 = M.Dot (_10, dx1, dy0); 	   // v = p dot h[_10]
		...
		public float _noise (Vector3 p)
		{
			// get constants 
			int[] h = Constants.hash;
			int[][] g = Constants.gradients3D;
			int m = Constants.hashMask;
			int gm = Constants.hashMask3D;
